...
import { AgentPurpose, PolicyEvaluationRequest, TrustTier } from './types';
...
async function runDemo() {
    ...
    const res1 = await phalaEntrypoint(JSON.stringify(authorizedPayload));
    ...
    const maliciousPayload: PolicyEvaluationRequest = {
        agent: baseAgent,
        action: {
    ...
    const res2 = await phalaEntrypoint(JSON.stringify(maliciousPayload));
